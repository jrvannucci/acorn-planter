...
```
acorn config set vscode_config_dir ./examples/vscode-config
...
`acorn vscode` is enough once `vscode_config_dir` is configured.

What lands where:

- **`settings.json`** is *merged* over acorn's own built-in defaults
  ...
  trimming, while every default acorn already sets (format-on-save,
...
re-running `acorn vscode` leaves a user's own edits to either file alone.

Unset it with `acorn config unset vscode_config_dir` when you're done trying
