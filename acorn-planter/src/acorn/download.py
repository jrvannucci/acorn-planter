"""
Shared, checksum-verifying download helper for everything acorn pulls
...
def extract_tar(archive: Path, dest: Path) -> None:
    ...
    explicitly pins ONE behavior across every interpreter acorn supports,
...
def fetch(url: str, dest: Path, *, expected_sha256: str | None = None,
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "acorn"})
