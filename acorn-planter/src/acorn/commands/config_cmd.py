"""
`acorn config` -- view and change acorn's own settings.
...
def show(args) -> int:
    data = config.load()
    print(f"acorn settings ({paths.CONFIG_FILE}):")
    print()
    ...
    if unknown:
        print()
        print(colors.warn("Keys in settings.json that acorn doesn't recognize:"))
        for key in unknown:
