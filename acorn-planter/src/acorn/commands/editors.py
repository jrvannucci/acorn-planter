...
acorn bundles one editor today (VS Code, in its Microsoft or VSCodium
...
def ensure_registered() -> dict[str, Editor]:
    ...
    reaches the registry without going through the CLI: `acorn.profile`
...
def confirm_first_install(args, *, label: str, note: str,
    ...
    is the common case and stays instant. Everything else in acorn that
...
def open_detached(argv: list[str], path: str) -> None:
    """Launch an editor at `path`, fully detached from acorn's own process
