"""
`acorn run [-n NAME] -- <cmd...>` -- run one command inside a venv, without
activating anything.

`acorn activate` deliberately mutates the calling shell, which is right for a
...
Three contracts matter more than the feature itself:
  ...
  2. **stdout and stderr are the child's, untouched.** The child inherits
     ...
     acorn's logging tee -- which means `acorn run -- python -c 'print
  ...
  3. **argv, not a shell.** No pipes, no redirection, no glob expansion, no
     shell=True. `acorn run -- sh -c '...'` is available to anyone who wants
...
USAGE = ("Usage: acorn run [-n <venv>] -- <command> [args...]\n"
         "   e.g. acorn run -- python -V\n"
...
def resolve_command(program: str, env: dict) -> str | None:
    ...
    This is load-bearing, and the reason `acorn run` doesn't just hand a bare
    ...
    child but is NOT used to find it. So `acorn run -- python` would prepend
...
def strip_separator(cmd: list[str]) -> list[str]:
    ...
    Only the first one: `acorn run -- pytest -- -k foo` must hand the second
...
def run(args) -> int:
    ...
    if program is None:
        ...
        print(f"error: command not found in venv '{target.name}': {cmd[0]}\n"
              f"       add it with:  acorn activate {target.name} "
              f"&& acorn install {cmd[0]}",
              file=sys.stderr)
        return 127

    try:
        # No capture and no check: the child owns the terminal, and a
        # non-zero exit is a result to pass on, not a acorn failure.
