"""
`acorn health-check` -- verifies every moving part acorn depends on and
...
WARN vs FAIL: FAIL means a core acorn operation would not work right now
...
def _check_uv() -> None:
    try:
        ...
        _fail("uv", "uv not found in ~/acorn/system/bin or on PATH -- re-run the installer")
        return
    ...
    for profile in profiles:
        ...
        for line in text.splitlines():
            ...
            if target is not None and target.exists():
                ...
                # old acorn layout) makes every new shell print an error.
                _warn("shell", f"stale acorn hook in {profile}: `{line.strip()}` "
...
def _render_text(data: dict) -> None:
    print(colors.bold("acorn health check") + f"  ({data['home']})")
    print()
