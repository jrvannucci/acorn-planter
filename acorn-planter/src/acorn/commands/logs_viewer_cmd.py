...
~/acorn/system/logs/ (acorn-YYYY-MM-DD.log), each a sequence of blocks:
...
the rest of acorn), and opens it. The page has search, a failures-only
...
# time as defense-in-depth -- e.g. a log written by an older/newer acorn
...
_PS_INSTALL_EXIT_RE = re.compile(
    r"acorn install (?:completed|FAILED) \(exit code (-?\d+)\)")
...
def _parse_install_file(path: Path) -> list[dict]:
    ...
    'acorn is installed.' success line, else remain unknown."""
    text = _read_log_text(path)
    ...
    elif "acorn is installed." in text:
        exit_code = 0
...
def run(args) -> int:
    ...
    if not entries:
        print("No commands logged yet -- the viewer will be empty until you "
              "run some `acorn` commands (or logging is off via ACORN_NO_LOG=1).")
