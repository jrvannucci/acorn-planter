...
opinion about built-in acorn command names -- collision detection against
...
def _exec(argv: list[str], *, env=None) -> int:
    ...
    contract as `run_cmd.run()`'s tail (`src/acorn/commands/run_cmd.py`),
...
def parse_startup_chain(entry: str) -> list[str]:
    ...
    ACORN_STARTUP_COMMANDS already reads it as "run the next one only if
...
def run_startup() -> int:
    ...
    for entry in entries:
        for name in parse_startup_chain(entry):
            cmd = index.get(name)
            if cmd is None:
                print(f"acorn: startup command '{name}' not found "
                      f"(declared in startup_commands, but not in "
                      f"custom-commands.toml)")
                break
            status = _run(cmd, [])
            if status != 0:
                print(f"acorn: startup command '{name}' failed "
                      f"(exit {status})")
                break
    return 0
...
def help_rows() -> list[tuple[str, str, str]]:
    ...
    `editors.help_rows()` (`src/acorn/commands/editors.py:150-170`).
