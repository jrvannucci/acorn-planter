"""
`acorn update-commands` -- the ONLY way acorn's own commands change.

The installers copy acorn's source into ~/acorn/system/src (WITHOUT
its .git folder -- no git checkout lives inside acorn) and record where
...
# Read-ACORNConf regex). One value per line, always double-quoted.
...
_DRIFT_CHECKS = [
    ("ACORN_VENV_DEFAULT_PACKAGES", "venv_default_packages", _drift_list),
    ("ACORN_PYTHON_MIRROR", "python_mirror", _drift_plain),
    ("ACORN_PACKAGE_INDEX", "package_index", _drift_plain),
    ("ACORN_CONDA_CHANNEL", "conda_channel", _drift_conda_channel),
    ("ACORN_NATIVE_TLS", "native_tls", _drift_native_tls),
    ("ACORN_VSCODE_FLAVOR", "vscode_flavor", _drift_vscode_flavor),
    ("ACORN_EXTENSION_GALLERY", "extension_gallery", _drift_plain),
    ("ACORN_VSCODE_EXTENSIONS", "vscode_extensions", _drift_vscode_extensions),
    ("ACORN_STARTUP_COMMANDS", "startup_commands", _drift_list),
]
...
def _swap_in(src: Path, tmp: Path) -> bool:
    """Replace ~/acorn/system/src with the freshly fetched copy at `tmp`.
...
def _refresh_from_directory(src: Path, source_dir: Path) -> bool:
    ...
    if not (source_dir / "src" / "pyproject.toml").exists():
        ...
        return False
    print(f"Copying acorn source from {source_dir} ...")
    tmp = src.parent / (src.name + ".new")
    fsutil.robust_rmtree(tmp)
    # .git never lives inside acorn, and vendor/ payloads (offline
...
def _refresh_from_url(src: Path, url: str, branch: str | None = None) -> bool:
    """Replace ~/acorn/system/src with a fresh shallow clone of `url`.
    ...
    try:
        ...
        print(f"git isn't available ({e}), so acorn can't download updates. "
    ...
    if branch:
        clone += ["--branch", branch]
        print(f"Downloading the latest acorn from {url} (branch {branch}) ...")
    else:
        print(f"Downloading the latest acorn from {url} ...")
    tmp = src.parent / (src.name + ".new")
    fsutil.robust_rmtree(tmp)
...
def _self_install_targets() -> list[Path]:
    ...
    return [paths.TOOL_DIR / "acorn", paths.BIN_DIR / exe]
...
def run(args) -> int:
    src = paths.SRC_DIR
    if not src.exists():
        print(f"No acorn source found at {src}.")
    ...
    if update_source:
        ...
        if source_dir.is_dir():
            ...
            # acorn from S:\..." that _refresh_from_url would print for
