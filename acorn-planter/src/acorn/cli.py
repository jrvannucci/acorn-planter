...
_HELP_GROUPS: list[tuple[str, list[tuple[str, str, str]]]] = [
    ("ACORN Status", [
        ("summary", "[--sizes] [--json]", "Show everything acorn has installed"),
        ("health-check", "[--json]", "Health-check the whole acorn install"),
        ("logs-viewer", "[--days N]", "Open the command logs in a browser"),
        ("where", "", "Print the acorn home directory"),
    ]),
    ...
    ("Git repos", [
        ("repo-clone", "<git-url>", "Clone a repo into ~/acorn/repo"),
    ...
    ("Utilities", [
        ...
        ("config", "[get|set|unset]", "View or change acorn settings"),
        ("kill-processes", "[name] [--system]", "Force-close acorn's processes (or all, or named)"),
    ...
    ("Danger zone -- these delete things (all support --preview)", [
        ...
        ("remove-user", "", "Delete everything acorn manages"),
        ("purge", "", "Full uninstall (also removes the shell hook)"),
        ("purge-and-reinstall", "", "Wipe acorn and reinstall it fresh from its original source"),
    ]),
]
...
def print_grouped_help(show_admin: bool = False) -> None:
    ...
    print(f"acorn {__version__} -- `acorn --version` prints this on its own.")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="acorn",
        description="acorn: a tidy, single-folder global Python "
                     "distributor and local environment manager.",
    )
    parser.add_argument("-V", "--version", action="version",
                        version=f"acorn {__version__}",
                        help="Print acorn's version and exit")
    ...
    danger.add_argument("--non-interactive", dest="non_interactive",
                        ...
                        help="Never wait for keyboard input: abort instead "
                             ...
                             "ACORN_NONINTERACTIVE=1 does the same.")
    ...
    p_venv_list = sub.add_parser(
        "venv-list", help="List every venv acorn has created")
    ...
    p_vscode.add_argument("--non-interactive", dest="non_interactive",
                           ...
                           help="Never wait for keyboard input: skip the "
                                ...
                                "-y to install). ACORN_NONINTERACTIVE=1 "
                                "does the same.")
    ...
    p_clone_repo = sub.add_parser(
        "repo-clone", help="Clone a git repo into ~/acorn/repo")
    ...
    p_cd_repo = sub.add_parser(
        "repo-cd", help="Change directory to a cloned repo (or ~/acorn/repo with no name)")
    ...
    p_open_repo = sub.add_parser(
        "repo-open", help="Open a cloned repo (or ~/acorn/repo) in the file manager")
    ...
    sub.add_parser("remove-user", parents=[danger],
                   help="Delete everything acorn manages")

    sub.add_parser("remove-venv-all", parents=[danger],
                   help="Delete every venv acorn has created")
    ...
    p_purge = sub.add_parser(
        ...
        help="Fully uninstall acorn: delete everything AND remove the shell hook")
    p_purge.add_argument("--keep-repos", action="store_true",
                          help="Move ~/acorn/repo out to safety before deleting everything else")
    ...
    p_update = sub.add_parser("update-commands",
                    help="Update the acorn CLI itself from its source in ~/acorn/system/src")
    ...
    sub.add_parser("where", help="Print the acorn home directory")

    p_summary = sub.add_parser(
        "summary", help="Show everything acorn has installed, on one screen")
    ...
    p_health = sub.add_parser(
        "health-check", help="Health-check the whole acorn install")
    ...
    p_config = sub.add_parser("config", help="View or change acorn settings")
    ...
    p_aru = sub.add_parser("admin-remove-user", parents=[danger],
                            help="[admin] Remove one user's entire acorn install")
...
def _dispatch_main(argv: list[str]) -> int:
    ...
    # environment here, so uv, git, and acorn's own downloads all
