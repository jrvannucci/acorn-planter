...
acorn's Python side is uv (PyPI world). This module is the *other* engine:
...
acorn never consults `defaults`: every install passes
...
always called by that exact path, keeping acorn self-contained.
"""
...
def tag_line(line: str) -> str:
    """Prefix micromamba's own output so it never reads as acorn's."""
    if not line.strip():
        return line
...
def find_micromamba() -> Path:
    ...
    raise MicromambaNotFound(
        "micromamba is not installed yet. `acorn forge-install` fetches it "
...
def channel_is_local(ch: str) -> bool:
    ...
    `acorn download-forge`) rather than a remote one. A bare name like
...
def solve_downloads(specs: list[str], channel_str: str,
    ...
    offline channel (`acorn download-forge`, and the offline bundler). Pass `mm`
    to use a specific micromamba (e.g. one just vendored into a bundle)."""
    argv = ["create", "--dry-run", "--json", "-n", "_acorn_solve",
...
def _env(extra: dict | None) -> dict:
    ...
    if extra:
        full.update(extra)
    # Keep micromamba's root prefix (envs + package cache) inside the acorn
