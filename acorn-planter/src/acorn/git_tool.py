...
(`acorn repo-clone`, and anything else that shells out to git) when no
system git is found.
...
def run_streamed(cmd: list[str]) -> int:
    ...
    acorn's command logging. Returns the exit code."""
...
def _latest_mingit_url() -> tuple[str, str | None]:
    ...
    req = urllib.request.Request(api_url, headers={"User-Agent": "acorn"})
    try:
...
def ensure_git() -> str:
    ...
    if system != "Windows":
        ...
        raise GitNotFound(
            f"git isn't installed, and acorn can't bundle a portable copy "
