...
shared-root install (ACORN_HOME_DIR="<root>/{user}").
...
def shared_root():
    """The directory that holds per-user acorn homes, or None if this
    isn't a shared multi-user install. Recorded at install time ONLY when
    ACORN_HOME_DIR used a {user} token -- so a plain ~/acorn never
...
def require_shared_root():
    ...
    if root is None:
        ...
        print("The admin commands manage a deployment where ACORN_HOME_DIR "
              "used a {user} token (e.g. C:\\acorn\\{user}), so each user "
...
def resolve_user_home(user: str):
    """The acorn home of another user under the shared root, or None."""
    root = shared_root()
    if root is None:
        return None
    candidate = root / user
...
def list_user_homes() -> list:
    """Every per-user acorn home under the shared root, sorted."""
    root = shared_root()
...
def all_user_profiles() -> list:
    """Every shell profile on the machine that might carry a acorn hook,
