...
def fake_micromamba(monkeypatch, home):
    ...
    monkeypatch.setattr(forge_cmd.conda_tool, "ensure_micromamba", fake_ensure)
    monkeypatch.setattr(forge_cmd.conda_tool, "run", fake_run)
    return calls


def _shim(name):
    return (paths.FORGE_SHIMS_DIR / (f"{name}.cmd" if os.name == "nt" else name))


def test_install_creates_env_shim_and_manifest(fake_micromamba, home, capsys):
    rc = forge_cmd.install(_ns(spec="ripgrep"))
    assert rc == 0
    ...
    manifest = json.loads(paths.forge_manifest_file("ripgrep").read_text())
...
def test_install_does_not_expose_python_runtime(fake_micromamba, home):
    forge_cmd.install(_ns(spec="ripgrep"))
...
def test_version_pin_names_the_env_after_the_package(fake_micromamba, home):
    forge_cmd.install(_ns(spec="ripgrep=14.1.0"))
    assert paths.forge_env_dir("ripgrep").exists()
    assert _shim("ripgrep").exists()
...
def test_remove_deletes_env_shim_and_manifest(fake_micromamba, home):
    forge_cmd.install(_ns(spec="ripgrep"))
    assert _shim("ripgrep").exists()
    rc = forge_cmd.remove(_ns(name="ripgrep", preview=False, yes=True))
    assert rc == 0
    assert not _shim("ripgrep").exists()
    assert not paths.forge_manifest_file("ripgrep").exists()
    assert not paths.forge_env_dir("ripgrep").exists()


def test_remove_aborts_without_confirmation(fake_micromamba, home, monkeypatch, capsys):
    """Like every other remove-* command, forge-remove needs confirmation:
    a 'no' answer leaves the env, shim, and manifest untouched."""
    forge_cmd.install(_ns(spec="ripgrep"))
    monkeypatch.setattr("builtins.input", lambda *a: "no")
    rc = forge_cmd.remove(_ns(name="ripgrep", preview=False))
    assert rc == 1
    assert "Aborted" in capsys.readouterr().out
    assert _shim("ripgrep").exists()
    assert paths.forge_manifest_file("ripgrep").exists()
    assert paths.forge_env_dir("ripgrep").exists()


def test_remove_preview_changes_nothing(fake_micromamba, home, capsys):
    forge_cmd.install(_ns(spec="ripgrep"))
    rc = forge_cmd.remove(_ns(name="ripgrep", preview=True))
    assert rc == 0
...
def test_remove_unknown(home, capsys):
    rc = forge_cmd.remove(_ns(name="ghost", preview=False))
    assert rc == 1
...
def test_run_tool_dispatches_to_the_right_env(fake_micromamba, home, monkeypatch):
    ...
    def fake_exec(env_name, command, toolargs):
        ...
        return 0
    monkeypatch.setattr(forge_cmd.conda_tool, "exec_tool", fake_exec)
    monkeypatch.setattr(forge_cmd.conda_tool, "find_micromamba",
                        lambda: paths.micromamba_binary())

    rc = forge_cmd.run_tool(_ns(name="ripgrep", toolargs=["foo", "--bar"]))
    assert rc == 0
...
def test_run_tool_propagates_exit_code(fake_micromamba, home, monkeypatch):
    forge_cmd.install(_ns(spec="ripgrep"))
    monkeypatch.setattr(forge_cmd.conda_tool, "exec_tool", lambda *a: 2)
    monkeypatch.setattr(forge_cmd.conda_tool, "find_micromamba",
                        lambda: paths.micromamba_binary())
    assert forge_cmd.run_tool(_ns(name="ripgrep", toolargs=[])) == 2


def test_run_tool_unknown_command(fake_micromamba, home, capsys):
    forge_cmd.install(_ns(spec="ripgrep"))
    capsys.readouterr()
    rc = forge_cmd.run_tool(_ns(name="nope", toolargs=[]))
    assert rc == 1
    out = capsys.readouterr().out
...
def test_run_tool_no_command_lists_available(fake_micromamba, home, capsys):
    forge_cmd.install(_ns(spec="ripgrep"))
    capsys.readouterr()
    rc = forge_cmd.run_tool(_ns(name=None, toolargs=[]))
    assert rc == 1
    assert "ripgrep" in capsys.readouterr().out


# --- download-forge (offline channel builder) ------------------------------
...
def fake_solve_and_fetch(monkeypatch):
    ...
    placeholder file), so download-forge's channel building is exercised with
    no network."""
    monkeypatch.setattr(forge_cmd.conda_tool, "ensure_micromamba",
                        lambda: paths.micromamba_binary())
    monkeypatch.setattr(forge_cmd.conda_tool, "solve_downloads",
...
def test_download_tool_builds_a_valid_channel(fake_solve_and_fetch, home, tmp_path):
    dest = tmp_path / "ch"
    rc = forge_cmd.download_tool(_ns(specs=["ripgrep"], dest=str(dest)))
    assert rc == 0
...
def test_download_tool_always_writes_a_noarch(fake_solve_and_fetch, home, tmp_path):
    ...
    forge_cmd.conda_tool.solve_downloads = lambda specs, ch: win_only
    dest = tmp_path / "ch2"
    forge_cmd.download_tool(_ns(specs=["ripgrep"], dest=str(dest)))
...
def test_download_tool_reports_offline_next_steps(fake_solve_and_fetch, home, tmp_path, capsys):
    forge_cmd.download_tool(_ns(specs=["ripgrep"], dest=str(tmp_path / "ch")))
    out = capsys.readouterr().out
    assert "conda_channel" in out and "forge-install ripgrep" in out
