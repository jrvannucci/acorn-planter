...
- `--python <tag>` selects which installed base Python to build from
  (matching a tag from `acorn python`). If omitted, uses the default base
...
```
acorn venv myproject
acorn venv myproject --python 311
```
...
each. The payload is byte-identical to the `venvs` array in `acorn summary
...
[Why `acorn` is a shell function](../GUIDE.md#why-acorn-is-a-shell-function)). Resolves
...
2. `VIRTUAL_ENV` — the venv active in this shell, the same one `acorn
...
The command name is resolved **in the venv**, so `acorn run -- python` is the
venv's python and `acorn run -- pytest` is the venv's pytest. (This is worth
...
`acorn summary --json` already answers the broader question.

## `acorn venv-default [name]`

Shows or sets the venv every **new** shell auto-activates on startup —
sugar for `acorn config get/set default_venv`, promoted to its own command
...
- With a name: validates the venv exists, then sets it. Existing shells
  are unaffected — open a new terminal (or `acorn activate <name>`).
- With no name: prints the current default (or that none is set).
- Clear it with `acorn config unset default_venv` — new shells then start
  with no venv active.

```
acorn venv-default
acorn venv-default myproject
```

## `acorn auto-activate [True|False]`

Turns **auto-activation of the default venv in new shells** on or off. This
is separate from *which* venv is the default (`acorn venv-default`): it decides
...
- Turning it off **leaves `default_venv` set** — `acorn activate` still works,
  and turning it back on resumes activating the same venv.

Sugar for the `auto_activate` setting (`acorn config set auto_activate
true|false`); on by default. The shell hook honours it without launching
acorn-cli — it detects the disabled state with a plain `grep` of
`settings.json`, which also lets it skip the startup acorn-cli call entirely
when auto-activation is off.

```
acorn auto-activate            # show current state
acorn auto-activate False      # stop auto-activating in new terminals
acorn auto-activate True       # resume
```

## `acorn install <package...>`
...
```
acorn install requests
acorn install -U "django>=5,<6" pillow
acorn install -e .                       # editable install of the current project
```

## `acorn uninstall <package...>`

Direct passthrough to `uv pip uninstall <package...>`, with the same
argument-forwarding and `VIRTUAL_ENV` warning behavior as `acorn install`.

```
acorn uninstall requests
```

## `acorn venv-licenses [-n NAME] [--all] [--json] [--fail-on FAMILY,...]`
...
([offline utilities](offline-utilities.md#acorn-whl-licenses-dir---all---json---fail-on-family)).

```
acorn venv-licenses            # the active venv
acorn venv-licenses -n analysis
```

Reads each distribution's installed `METADATA`. No network, nothing
installed, no third-party dependency.

## `acorn package-list [--json]`
...
package that's installed, the read-only counterpart of `acorn install`. Same
...
gone (see [Why `acorn` is a shell function](../GUIDE.md#why-acorn-is-a-shell-function)).
...
```
acorn remove-venv myproject
acorn remove-venv myproject -y
```
