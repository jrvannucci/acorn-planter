...
seedling by default** — bare `acorn kill-processes` only force-closes
...
- `acorn kill-processes` (no arguments) — seedling's own processes only.
...
```
acorn kill-processes                # just seedling's own stuck processes
...
(`~/seedling/system/shell/acorn.ps1` / `acorn.sh`) from the refreshed
