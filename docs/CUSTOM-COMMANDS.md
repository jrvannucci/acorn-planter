...
builds*. Custom commands say *what extra verbs `acorn` itself understands*.
They're separate mechanisms because they're read differently: a profile is
only consulted when someone runs `acorn apply`; custom commands have to be
resolved on essentially every `acorn custom ...` (and, for `toplevel` ones,
every bare `acorn <name>`) invocation.
...
- **`script = "..."`** — a `.py`, `.sh`, or `.ps1` file, resolved relative to
  ...
  logic, a companion data file, or that needs to chain more than one `acorn`
...
- `name` — what you type after `acorn custom`. Letters, digits, `.`, `_`,
  `-`; must start with a letter or digit.
- `run` — the argv to execute. A list of strings, **never a shell string**
  (no `shell=True`, no `&&`, no globbing) — the same stance `acorn run`
...
- `venv` (optional, `run` only) — run inside this **named** venv (resolved
  ...
  makes `run = ["acorn", "install", ...]` with no `venv` mean "into the
  currently activated venv" (see [worked examples](#worked-examples)).
- `description` (optional) — shown in `acorn custom` and `acorn help`.
- `toplevel` (optional, default `false`) — see [making a command
  top-level](#making-a-command-top-level).

Trailing arguments are appended to `run` verbatim: `acorn custom lint --fix`
runs `ruff check . --fix`.
...
both) — a script already runs in acorn-cli's own interpreter/shell, and
reaches a venv itself, on its own terms, via `acorn run -n <venv> -- ...`.

**Orchestration scripts don't get a special API.** `acorn` is already a full,
composable CLI — a script chains operations by shelling out to it, the
same thing `acorn apply` already does internally:
...
Stick to the standard library plus `acorn` subprocess calls — these scripts
run in acorn-cli's own interpreter, not a project venv, so they were never
...
and the trick is that it can invoke `acorn` itself:

```toml
[[command]]
name = "data-stack"
run = ["acorn", "install", "pandas", "numpy", "matplotlib", "scikit-learn"]
description = "Install the standard data-science package set into the active venv"
```

No `venv` key: plain `acorn install` already targets whatever's active, so
omitting `venv` here and calling `acorn install` *is* "into the currently
activated venv." `acorn custom data-stack --upgrade` becomes `acorn install
...
Most custom commands are reached with `acorn custom <name>`. A command can
**also** run as bare `acorn <name>` with `toplevel = true` — for `run` and
...
time `acorn custom` or `acorn help` runs, so you notice and rename it. A
...
```
acorn config set startup_commands "check-mirror, motd"
```
...
```
acorn config set startup_commands "ensure-venv&&sync-data, motd"
```
...
**One acorn-cli process runs the lot**, so a five-command routine costs one
...
read fresh from wherever it's configured on every `acorn custom`/`acorn help`
invocation (no caching) — a conf-sourced path inside the distributed repo
rides along automatically whenever that repo copy is refreshed by `acorn
...
`settings.json` the same way, and read by the shell hook (`acorn.ps1`/
...
```
acorn config set custom_commands ./team-commands.toml
acorn config set startup_commands "check-mirror,motd"
```
...
| `[[command]] description` | string | Shown in `acorn custom` / `acorn help`. |
| `[[command]] toplevel` | bool | Also expose as bare `acorn <name>`. Default `false`. |
...
broken `custom-commands.toml` only breaks `acorn custom` and the `acorn help`
group — it can never break an unrelated command like `acorn venv`.
