        ...
        (None, [
            ("acorn python [version]",
             ["Installs a base CPython interpreter via uv. The first one",
              "installed becomes the default `acorn venv` builds from."], False, [("python", "base")]),
    ...
    ("venvs-and-packages", "Venvs & packages",
     ...
     [
        ...
        ("Packages", [
            ...
            ("acorn venv-licenses [-n name]",
             ["What every package installed in a venv is licensed under, and",
              "which ones carry an obligation worth a decision."], False, [("python", "venvs")]),
            ("acorn show <package...>",
             ["Passthrough to `uv pip show` -- full detail on one installed package."], False, [("python", "venvs")]),
        ]),
        ("Danger zone", [
            ("acorn remove-venv <name>",
    ...
    ("offline-utilities", "Offline utilities",
     ...
     [
        (None, [
            ...
            ("acorn download-requirements <req.txt>",
             ["Same as download-whls, reading specifiers from a requirements file."], False, [("outside", "wheelhouse")]),
            ("acorn download-forge <name...>",
             ["Resolves a conda-forge tool and its dependencies into a local",
              "conda channel folder -- the download-whls of the conda-forge side."], False, [("outside", "conda-channel")]),
            ("acorn whl-licenses <dir>",
             ["What every wheel in a directory is licensed under, read from",
              "each wheel's own metadata -- before it ships to a share."], False, [("outside", "wheelhouse")]),
            ("acorn forge-licenses [dir]",
             ["The same for a bundled conda channel, read from its repodata."], False, [("outside", "conda-channel")]),
            ("acorn upload-whls <dir>",
             ["Publishes a wheel directory INTO your internal package index,",
              "for networks whose air gap has a door. Runs uvx twine upload."], False, [("outside", "wheelhouse")]),
        ]),
     ]),
    ...
    ("repos", "Repos",
     ...
     [
        (None, [
            ...
            ("acorn repo-install <name>[extras]",
    ...
    ("editors", "Editors & IDEs",
     ...
     [
        (None, [
            ...
            ("acorn spyder-repo <name> [--venv name]",
             ["Opens a cloned repo as a Spyder project -- same install-if-needed", "shape as acorn spyder."], False, [("extensions", "apps"), ("system", "shims"), ("extensions", "spyder-config")]),
        ]),
     ]),
        ...
        (None, [
            ("acorn custom [name] [args...]",
             ["Runs one [[command]] entry from custom-commands.toml: a fixed",
              "argv (run = [...]), or a script that chains acorn subcommands",
        ...
        (None, [
            ...
            ("acorn update-commands",
             ["Updates acorn itself from its recorded source, and reports any",
              "global.conf drift since install -- never touches venvs, packages, or repos."], False, [("system", "src"), ("system", "shell")]),
        ]),
        ("Danger zone", [
            ("acorn remove-user",
