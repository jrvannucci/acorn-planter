...
acorn installs from them the same way it would from the public ones. It is
...
![Two origins, both internal: artifactory.corp.example alone serves packages, Spyder, conda-forge tools AND interpreters, while gitlab.corp.example serves acorn itself plus repos -- no offline-bundle/ anywhere.](../diagrams/profile-pull-internal-mirrors.svg)
...
ACORN_CONDA_CHANNEL="https://artifactory.corp.example/api/conda/conda-forge"
ACORN_PYTHON_MIRROR="https://artifactory.corp.example/generic/python-build-standalone"

# Install acorn itself from the internal git host.
ACORN_REPO_URL="https://gitlab.corp.example/platform/acorn.git"

# The proxy re-signs HTTPS. Either trust the OS store, if IT installed the
# corporate root machine-wide...
ACORN_NATIVE_TLS="true"
...
- **`package_index` as a URL, not a directory.** acorn treats the two
  ...
  so it's a URL.
- **`ACORN_NATIVE_TLS="true"` is usually the right answer** on a managed
  fleet — IT has already pushed the corporate root into the OS store, so
  acorn can use it and you ship no certificate at all. Fall back to
...
- **`ACORN_AUTO_VSCODE="false"`** is belt-and-braces: the profile's
...
ACORN_STARTUP_COMMANDS="check-mirror"
```
...
can already reach, and acorn installs from them exactly as it would from
...
- `ACORN_NATIVE_TLS="true"` if IT already installed the corporate root
  machine-wide — acorn then uses the OS trust store and you ship nothing.
