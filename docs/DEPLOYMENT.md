...
- `SEEDLING_HOME_DIR` (default: `~/seedling`) — the folder everything
  ...
  exports the resolved `SEEDLING_HOME` so acorn-cli finds it at runtime.)
...
- `SEEDLING_PYTHON_MIRROR` (default: empty = internet) — where `acorn
...
- `SEEDLING_CONDA_CHANNEL` (default: `conda-forge`) — where `acorn
...
1. **Pick the install source.** A git URL if you have a self-hosted git
   ...
   `SEEDLING_REPO_URL`, and it is also where `acorn update-commands` will
...
```
acorn admin-remove-user alice          # removes C:\seedling\alice entirely
```
