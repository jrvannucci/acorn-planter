...
makes Spyder actually usable against a acorn venv, which a generic
application install cannot know to do:
  ...
  1. Containment. Spyder writes its settings to ~/.config/spyder-6 (or
     ...
     stays inside ~/acorn, so `acorn purge` really does leave nothing.
...
def resolve_venv(args=None) -> tuple[object | None, str | None]:
    ...
    VIRTUAL_ENV is honored even when it points outside ~/acorn, because
...
def _warn_if_downgraded(venv_name: str, before: str | None,
    ...
    spyder-kernels caps ipykernel below 7, while acorn's default venv
    ...
    notebooks), and everything else in acorn that costs something says so
...
def _launch(path: str, *, as_project: bool) -> None:
    """Open Spyder, detached, with its config kept inside acorn."""
