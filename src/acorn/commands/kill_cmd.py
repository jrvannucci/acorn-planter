...
Scoped by DEFAULT to processes belonging to the acorn tree; `--system` is
...
"Belonging to acorn" is decided by LOCATION -- executable path, command
line, or working directory under the acorn home -- never by process name.
...
consistent with acorn's "nothing pre-installed required" design.

Always excludes acorn's own running process (and its parent) so it can't
...
def find_acorn_processes(root: str | None = None) -> list[tuple[int, str]]:
    """[(pid, name)] for processes tied to the acorn tree -- by executable
...
def kill_python_and_vscode() -> list:
    """Force-closes every Python/VS Code process, sparing acorn's own.
    Shared by `acorn kill-processes --system` and `acorn remove-user` (which uses
    this to release any file locks before deleting ~/acorn)."""
    exclude = _self_and_parent()
    if platform.system() == "Windows":
...
def kill_acorn_processes() -> list[tuple[int, str]]:
    """Close only the processes tied to the acorn tree. Returns what was
    actually closed, as (pid, name)."""
    found = find_acorn_processes()
    if not found:
        return []
...
def run(args) -> int:
    ...
    if system_wide:
        ...
        description = "acorn's own processes"
    ...
    if confirm.preview_requested(args):
        if system_wide:
            ...
            items = [f"{name} (pid {pid})"
                     for pid, name in find_acorn_processes()]
    ...
    if not confirm.auto_confirmed(args):
        ...
        if system_wide or target:
            ...
            print("not just acorn's -- including any unsaved work.")
            print("Use `acorn kill-processes` with no arguments to close only "
                  "acorn's.")
        else:
            print(f"This force-closes {description} " +
                  colors.warn("(anything running from a acorn venv or repo)") +
                  ",")
