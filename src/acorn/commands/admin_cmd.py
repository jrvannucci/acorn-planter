...
def _report(failures: list) -> int:
    if failures:
        ...
        for f in failures:
            print(f"  - {f}")
        print("Something outside acorn may still hold them open (a logged-in "
...
def purge_all_users(args) -> int:
    """admin-purge-all-users -- remove every user's acorn home under the
    ...
    if not homes:
        print(f"No per-user acorn installs found under {root}. Nothing to do.")
        return 0
    ...
    if not confirm.auto_confirmed(args):
        print(colors.danger(f"This removes EVERY user's acorn install under {root}:"))
        for h in homes:
            print(f"  - {h}")
    ...
    print()
    if stripped:
        print(f"Removed the acorn shell hook from {stripped} user profile(s).")
    print(colors.ok("All-users purge complete."))
    return _report(failures)


def remove_user(args) -> int:
    """admin-remove-user <user> -- remove one user's entire acorn home."""
    if not admin.require_elevation():
        return 1
    ...
    if home is None:
        print(f"No acorn install for user '{user}' under {admin.shared_root()}.")
        return 1
    return _confirm_and_delete(args, f"remove {user}'s entire acorn install",
...
def _strip_all_hooks(shared_root) -> int:
    """Remove acorn hook lines from every user's profile. A hook line
