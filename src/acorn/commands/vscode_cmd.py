...
class UnknownFlavor(ValueError):
    """Raised when vscode_flavor names a build acorn doesn't know."""
...
def _resolve_vscodium() -> tuple[str, str | None]:
    ...
    req = urllib.request.Request(
        _VSCODIUM_RELEASE_API, headers={"User-Agent": "acorn"})
...
def _resolve_download(os_id: str, kind: str, name: str) -> tuple[str, str | None]:
    ...
    try:
        req = urllib.request.Request(api, headers={"User-Agent": "acorn"})
        with urllib.request.urlopen(req) as resp:
            data = json.loads(resp.read())
        url = data.get("url")
        if url:
...
def _write_default_settings() -> None:
    ...
    if not settings_file.exists():
        ...
        # Pointing it at acorn's own venvs folder is what makes every
        ...
        # in LAST, so its values win over both acorn's DEFAULT_SETTINGS
...
def open_window(cli: list[str], path: str) -> None:
    ...
    acorn's own process. Kept as the name `acorn vscode` and `acorn
    vscode-repo` already call; the detaching itself is family-generic."""
    editors.open_detached(cli, path)


def is_installed() -> bool:
    """Whether a usable VS Code is already unpacked in the acorn tree.
