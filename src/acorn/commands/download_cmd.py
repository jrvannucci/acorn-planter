...
exactly what acorn's `package_index` setting consumes on the offline side:
...
work without acorn needing to know about them.
...
# directory (not ~/acorn) because a bundle is meant to be carried OFF this
...
def _index_and_cert_args() -> list[str]:
    """Translate acorn's `package_index` / `ca_cert` settings into the pip
...
def run_upload(args) -> int:
    ...
    if ca:
        ca_path = Path(str(ca)).expanduser()
        if ca_path.is_file():
            # Same corporate-CA story as every other network call acorn
    ...
    # visible to `ps`, lands in shell history, and is echoed into acorn's
