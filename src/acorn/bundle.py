...
# Every bundle holds these whether or not it says so: acorn itself is built
...
def parse(text: str, path: Path | None = None) -> Bundle:
    ...
    _require(isinstance(schema, int) and schema <= SCHEMA,
             f"schema {schema!r} is newer than this acorn understands "
             f"(supported: {SCHEMA}). Update acorn first.")
    ...
    for name in b.platforms:
        _require(name.strip().lower() in PLATFORM_TAGS,
                 f"platforms: {name!r} isn't one acorn knows. Valid: "
...
def check_conf(bundle: Bundle, conf: dict[str, str]) -> list[str]:
    ...
    flavor = (conf.get("ACORN_VSCODE_FLAVOR") or "").strip().lower()
    if flavor and flavor != bundle.editor_flavor:
        problems.append(
            f"editor flavor: the bundle stages {bundle.editor_flavor!r} but "
            f"global.conf sets ACORN_VSCODE_FLAVOR={flavor!r}")

    gallery = (conf.get("ACORN_EXTENSION_GALLERY") or "").strip()
    ...
    declared = (conf.get("ACORN_VSCODE_EXTENSIONS") or "").strip()
