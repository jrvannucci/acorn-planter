...
`global.conf` answers "where does acorn get things from" and is read by
the shell installers. A profile answers "what should be set up once acorn
...
comments; `tomllib` is stdlib on the 3.12 floor acorn already requires, so
...
class Profile:
    ...
    # ACORN_AUTO_VSCODE setting keeps deciding that, so a profile written
...
def parse(text: str, *, path: Path | None = None) -> Profile:
    ...
    _require(schema <= SCHEMA,
             f"profile declares schema {schema}, but this acorn "
             f"understands up to {SCHEMA}. Update acorn first "
...
def current_user() -> str:
    ...
    the {user} token in ACORN_HOME_DIR expands to, so a shared-root
