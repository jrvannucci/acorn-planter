"""
Per-command logging. Every `acorn` invocation appends to a daily file under
~/acorn/system/logs/ (e.g. acorn-2026-07-05.log): the exact command line,
...
terminal untouched (colors, isatty behavior, and the `acorn activate
...
or written (locked, disk full, permissions), acorn silently carries on
without it. Set ACORN_NO_LOG=1 to disable logging entirely.
"""
...
def _redacted(argv: list[str]) -> list[str]:
    ...
    `acorn config set <key> <value>` carries the value on the command line, and
...
def start(argv: list[str]) -> None:
    ...
    if os.environ.get("ACORN_NO_LOG") == "1" or _logfile is not None:
        return
    try:
        paths.LOGS_DIR.mkdir(parents=True, exist_ok=True)
        today = _dt.date.today().isoformat()
        _logfile = open(paths.LOGS_DIR / f"acorn-{today}.log", "a", encoding="utf-8")
        now = _dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        _logfile.write(f"\n=== [{now}] acorn {' '.join(_redacted(argv))}\n")
        _logfile.flush()
    except OSError:
        _logfile = None
        return
...
def close_before_deleting_home() -> None:
    ...
    `purge`). Without this, this process still has acorn-YYYY-MM-DD.log open
