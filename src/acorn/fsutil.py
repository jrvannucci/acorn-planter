"""
Shared, defensive directory deletion, used everywhere acorn deletes a
...
3. Read-only files fail deletion on Windows outright -- and git marks every
   ...
   (cloned repos, acorn's own source copy) hits this on every single
...
ESCALATION_NOTE = (
    "nothing is closed up front -- if a file turns out to be locked, acorn "
...
def remove_tree(path: Path, *, label: str = "", allow_sledgehammer: bool = True,
    ...
    if not holders:
        ...
        scoped = kill_cmd.find_acorn_processes(str(path))
...
def failures_are_only_running_cli(failures: list[str], home: Path) -> bool:
    """True when everything robust_rmtree couldn't delete is acorn's own
...
def schedule_deferred_delete(path: Path) -> None:
    ...
    if os.name == "nt":
        ...
        import tempfile
        bat = Path(tempfile.gettempdir()) / f"acorn-cleanup-{os.getpid()}.bat"
        bat.write_text(
            "@echo off\r\n"
        ...
        # when finished.
        import tempfile
        marker = Path(tempfile.gettempdir()) / "acorn-cleanup.pending"
        try:
