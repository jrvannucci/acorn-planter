...
drives acorn -- CI matrix jobs, a profile being applied while someone
...
mechanisms used here are stdlib and present everywhere acorn runs:
`msvcrt.locking` on Windows, `fcntl.flock` elsewhere. Nothing to install,
which is acorn's rule.
...
def file_lock(key: str, what: str, timeout: float = DEFAULT_TIMEOUT):
    ...
    trade-off matches how acorn already treats its logs.
    """
    try:
...
def venv_lock(venv_path, timeout: float = DEFAULT_TIMEOUT):
    ...
    wherever it points, including outside ~/acorn, and two unrelated
...
def active_venv_lock(timeout: float = DEFAULT_TIMEOUT):
    ...
    directory -- outside acorn's world and not its to serialize -- so
