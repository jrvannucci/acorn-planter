...
Three knobs, honored everywhere:
  ...
  - `-y`/`--yes` (or ACORN_YES=1) skips the prompt and proceeds.
  - `--non-interactive` (or ACORN_NONINTERACTIVE=1) means "never sit
    waiting for keyboard input": anything that would have prompted instead
    aborts safely, unless -y/ACORN_YES was also given. This is the mode
...
def auto_confirmed(args) -> bool:
    if getattr(args, "yes", False):
        return True
    return os.environ.get("ACORN_YES") == "1"


def non_interactive(args) -> bool:
    if getattr(args, "non_interactive", False):
        return True
    return os.environ.get("ACORN_NONINTERACTIVE") == "1"
...
def confirm(args, prompt: str = "") -> bool:
    ...
    if non_interactive(args):
        print("Non-interactive mode: refusing to prompt for confirmation. "
              "Pass -y/--yes (or set ACORN_YES=1) to proceed.")
        return False
...
def ask(args, question: str) -> bool:
    ...
    the thing you just asked for" it reads as acorn being obstinate.
