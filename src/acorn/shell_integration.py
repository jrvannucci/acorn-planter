...
_PLACEHOLDER = "__ACORN_HOME_PLACEHOLDER__"
...
_HOME_LINE_RE = {
    "acorn.ps1": re.compile(r'^\$script:ACORNHome\s*=\s*"(.+)"\s*$', re.MULTILINE),
    "acorn.sh": re.compile(r'^__ACORN_HOME="(.+)"\s*$', re.MULTILINE),
}
...
def _templates_dir() -> Path:
    ...
    the repo: system/src/src/acorn/shell/. `acorn update-commands` swaps a
    fresh copy in before calling refresh(), so this is the UPDATED template."""
    return paths.SRC_DIR / "src" / "acorn" / "shell"
...
def ensure_bin_on_windows_path() -> bool:
    ...
    ACORN_SKIP_PATH_REGISTER is the same test-only escape hatch
    ...
    if os.name != "nt" or os.environ.get("ACORN_SKIP_PATH_REGISTER"):
        return False
    import winreg
